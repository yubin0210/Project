...
<main class="frame">
	...
	<form method="POST" enctype="multipart/form-data" class="program-add-form">
		...
		<div class="sb">
			...
			<p><input type="text" name="title" placeholder="프로그램 이름" required autofocus></p>
		</div>
		<div class="sb">
		...
		<div>
			<p><textarea name="content" rows="4" cols="50" placeholder="프로그램 설명"></textarea></p>
		</div>
		<div align="center"><input type="submit" value="프로그램 등록"></div>
	</form>
	
	</c:if>
	
	<c:if test="${pageContext.request.method == 'POST' }">
		<c:set var="dto" value="${fileUtil.getProgramDTO(pageContext.request) }" />
		<c:set var="row" value="${programDAO.insert(dto) }" />
